...
- **iOS "local network" prompt + blank app = a dev URL leaked into the release bundle.** This has bitten us several times. Symptom: a release/TestFlight build shows the "find devices on your local network" prompt and the map + profile never load (every request stalls ~12s then fails). Root cause is NOT the ATS config — it's `EXPO_PUBLIC_API_URL` / `EXPO_PUBLIC_AUTH_URL` getting inlined as a `*.local` host (e.g. `https://app.devrijehond.local`) or `localhost`. The app then resolves a Bonjour/mDNS name, which needs Local Network permission and gates DNS. Confirm via the device log: `idevicesyslog` and grep for `app.devrijehond.local` / `getaddrinfo` stalls / `ATSAllowsLocalNetworking`. Guardrails, all three must hold:
  1. The repo-root `.env.local` must NOT contain `EXPO_PUBLIC_*`. Mobile public URLs live only in `apps/mobile/.env` (prod) and `apps/mobile/.env.development.local` (local). `scripts/release-ios.sh` sources the root `.env.local` for the ASC keys, so anything there leaks into the bundle.
  2. `scripts/release-ios.sh` force-exports the prod `EXPO_PUBLIC_API_URL`/`AUTH_URL` _unconditionally_ (not `${VAR:-default}`, which can't override an already-exported value).
  3. `apps/mobile/lib/config.ts` `isLocalUrl()` rejects `localhost`, private LAN ranges, and `*.local`, forcing `DEFAULT_API_URL` in any release (`!__DEV__`) build — the last-line safety net.
