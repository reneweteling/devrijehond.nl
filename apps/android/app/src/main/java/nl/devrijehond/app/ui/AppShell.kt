...
fun AppShell() {
    ...
    Scaffold(
        ...
        bottomBar = {
            if (onTab) {
                // A hairline above the bar so it reads as a docked surface against the
                // map/sand instead of blending in. The NavigationBar itself owns the
                // bottom navigation-bar inset (drawn in its container colour), so the
                // bar sits flush above the gesture pill with no extra padding.
                Column {
                    HorizontalDivider(thickness = 1.dp, color = Brand.Ink.copy(alpha = 0.08f))
                    NavigationBar(
